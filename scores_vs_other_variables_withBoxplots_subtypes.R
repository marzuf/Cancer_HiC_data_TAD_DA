startTime <- Sys.time()
cat(paste0("> Rscript scores_vs_other_variables_withBoxplots_subtypes.R\n"))

#  Rscript scores_vs_other_variables_withBoxplots_subtypes.R
...
if("TCGAskcm_wt_mutCTNNB1" %in% basename(all_datasets)){ 
  outFold <- file.path("SCORES_VS_OTHER_VARIABLES_WITH_BOXPLOTS_SUBTYPES")
}else {
  outFold <- file.path("SCORES_VS_OTHER_VARIABLES_WITH_BOXPLOTS_NOOUT_SUBTYPES")
}
...
load("SCORES_VS_OTHER_VARIABLES_WITH_BOXPLOTS_SUBTYPES/datasets_variables_DT.Rdata")

head(datasets_variables_DT)

avTypeDS_DT <- aggregate(dsType ~ hicds, function(x) length(unique(x)), data=datasets_variables_DT)

manyTypes_DT <- datasets_variables_DT[datasets_variables_DT$hicds %in% avTypeDS_DT$hicds[avTypeDS_DT$dsType > 1],]
...
auc_vars <- c("fccAUC", "coexprDistAUC")

var_plot = "fccAUC"

for(var_plot in auc_vars) {
  
  var_name <- titNames[var_plot]
  
  manyTypes_DT <- manyTypes_DT[order(manyTypes_DT[, var_plot]),]
  
  meanhicds_DT <- aggregate(as.formula(paste0(var_plot, " ~ hicds")), data=manyTypes_DT, FUN=mean, na.rm=T)
  meanhicds_DT <- meanhicds_DT[order(meanhicds_DT[, var_plot]),]
  ...
  pSubtype <-  ggplot(manyTypes_DT, aes_string(x="dataset", y=var_plot, fill = "dsType"))+
    ...
    outFile <- file.path(outFold, paste0(var_plot, "_bySubtype", "_barplot.", plotType))
    ggsave(pSubtype, file=outFile, height=myHeightGG, width=myWidthGG)
    cat(paste0("... written: ", outFile, "\n"))
