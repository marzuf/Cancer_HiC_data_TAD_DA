...
all_hic_ds <- list.files(dsFold, full.names=TRUE)
stopifnot(length(all_hic_ds) > 0)
txt <- paste0("... found # hic_ds:\t", length(all_hic_ds), "\n")
printAndLog(txt, logFile)

all_hic_expr_ds <- sapply(all_hic_ds, list.files)
names(all_hic_expr_ds) <- basename(all_hic_ds)
txt <- paste0("... found # hic_expr_ds:\t", length(unlist(all_hic_expr_ds)), "\n")
printAndLog(txt, logFile)
...
if(buildTable) {
  hic_ds = basename(all_hic_ds[1])
  # "ENCSR079VIJ_G401_40kb"
  all_ds_DT <- foreach(hic_ds = basename(all_hic_ds), .combine='rbind') %dopar% {
    txt <- paste0("*** START:\t", hic_ds, "\n")
    printAndLog(txt, logFile)
    
    stopifnot(dir.exists(hic_ds))
    
    gene2tadDT_file <- file.path(hic_ds, "genes2tad", "all_genes_positions.txt")
    ...
    all_curr_expr_ds <- all_hic_expr_ds[[hic_ds]]
    
    curr_expr_ds = all_curr_expr_ds[1]
    
    expr_ds_DT <- foreach(curr_expr_ds = all_curr_expr_ds, .combine='rbind') %do% {
      
      ### RETRIEVE TAD PVALUES
      step11_fold <- file.path(dsFold, hic_ds, curr_expr_ds, "11_runEmpPvalCombined")
      ...
      step1_fold <- file.path(dsFold, hic_ds, curr_expr_ds, "1_runGeneDE")
      ...
      step0_fold <- file.path(dsFold, hic_ds, curr_expr_ds, "0_prepGeneData")
      ...
      ### RETRIEVE FCC
      step17_fold <- file.path(dsFold, hic_ds, curr_expr_ds, "170revision2EZH2_score_auc_pval_permGenes")
      aucFCC_file <- file.path(step17_fold, "auc_ratios.Rdata")
      stopifnot(file.exists(aucFCC_file))
      
      # AUC_COEXPRDIST_WITHFAM_SORTNODUP/K562_40kb/TCGAlaml_wt_mutFLT3_hgnc/hgnc_family_short/
      
      aucCoexprDist_file <- file.path("AUC_COEXPRDIST_WITHFAM_SORTNODUP", hic_ds, paste0(curr_expr_ds, "_hgnc"),
                                      "hgnc_family_short", "auc_values.Rdata")
    ...
    } # end iterating over expr_ds for the current hic_ds
    head(expr_ds_DT)
    str(expr_ds_DT)
    if(is.null(nrow(expr_ds_DT))){
      stopifnot(length(all_curr_expr_ds) == 1)
      expr_ds_DT <- t(as.data.frame(expr_ds_DT))
    }
    rownames(expr_ds_DT) <- all_curr_expr_ds
    expr_ds_DT <- as.data.frame(expr_ds_DT)
    stopifnot(apply(expr_ds_DT, 2, is.numeric))
    colnames(expr_ds_DT) <- c(
      "nbrTADs",
      ...
      "ratioAUC_coexprDist")  
    stopifnot(apply(expr_ds_DT, 2, is.numeric))
    expr_ds_DT <- expr_ds_DT[order(expr_ds_DT$ratioAUC_FCC, decreasing=T),,drop=F]
    expr_ds_DT$ratioAUC_FCC<- round(expr_ds_DT$ratioAUC_FCC, 4)
    expr_ds_DT$ratioAUC_coexprDist <- round(expr_ds_DT$ratioAUC_coexprDist, 4)
    expr_ds_DT <- cbind(data.frame(hic_dataset=hic_ds, expr_dataset=rownames(expr_ds_DT), stringsAsFactors = FALSE),
                        expr_ds_DT)
    expr_ds_DT
