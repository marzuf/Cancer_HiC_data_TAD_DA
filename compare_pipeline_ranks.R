...
cat("> START compare_pipeline_ranks.R", "\n")
...
outFold <- file.path("COMPARE_PIPELINE_RANKS", paste0(ds1, "_", ds2), exprDS)
...
ds1 <- paste0(ds1, folderSuffix)
ds2 <- paste0(ds2, folderSuffix)
stopifnot(dir.exists(ds1))
stopifnot(dir.exists(ds2))

############################## DATASET 1
stopifnot(ds1 != "pipelineConsensus")
mainDir1 <- file.path("PIPELINE/OUTPUT_FOLDER", ds1, exprDS)
stopifnot(dir.exists(mainDir1))

g2t_file1 <- file.path(ds1, "genes2tad/all_genes_positions.txt")
stopifnot(file.exists(g2t_file1))
...
g2t_file2 <- file.path(ds2, "genes2tad/all_genes_positions.txt")
stopifnot(file.exists(g2t_file2))

if(ds2 == paste0("pipelineConsensus", folderSuffix)) {
  mainDir2 <- file.path(setDir, "/mnt/ed4/marie/scripts/TAD_DE_pipeline_v2_TopDom/OUTPUT_FOLDER", exprDS)
} else {
  mainDir2 <- file.path("PIPELINE/OUTPUT_FOLDER", ds2, exprDS)
}
