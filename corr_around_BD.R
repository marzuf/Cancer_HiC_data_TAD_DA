...
for(ds in all_hicexpr_ds) {
  ...
  g2t_DT <- g2t_DT[g2t_DT$entrezID %in% pipeline_geneList,,drop=FALSE]
  ...
  allBD_nGenes_corr <- foreach(i_bd = seq_len(nPos)) %dopar% {
    ...
    curr_genesRightDT <- curr_genesRightDT[order(curr_genesRightDT$posDiff),,drop=FALSE]
    curr_genesLeftDT <- curr_genesLeftDT[order(curr_genesLeftDT$posDiff),,drop=FALSE]
    ...
    for(nGenes in all_gene_nbrs) {
      ...
      bd_all_nGenes_corr[as.character(nGenes)] <- meanCorr_rightLeft
      
    } # end for-iterating over gene #
    stopifnot(length(bd_all_nGenes_corr) == length(all_gene_nbrs))
    stopifnot(!is.na(bd_all_nGenes_corr))
